...
function loadGauges() {
    for (let gaugeId of Object.keys(weewxData.gauges)) {
        ...
        let minvalue = round(convert(gauge.weewxData, gauge.weewxData.minvalue, gauge.weewxData.target_unit), gaugePitchPrecision);
        let maxvalue = round(convert(gauge.weewxData, gauge.weewxData.maxvalue, gauge.weewxData.target_unit), gaugePitchPrecision);
        ...
        if (gauge.weewxData.obs_group === "group_direction") {
            ...
            for (let i = 0; i < lineColors.length; i++) {
                ...
                if (isNaN(untilValue)) {
                    ...
                    untilValue = round(convert(gauge.weewxData, untilValue, gauge.weewxData.target_unit), gaugePitchPrecision);
                }
