...
class GaugeDraw(ImageDraw.ImageDraw):
    ...
    def __init__(self, im, min_val, max_val, dial_range=270, background_color=None, offset_angle=0):
        ...
        self.gauge_value2 = None
        self.gauge_value3 = None
        
        self.marker_value = None
        self.marker_value2 = None
        self.marker_value3 = None
        
        # Text caption will be stored here
        self.text_labels = None
        self.text_labels2 = None
        self.text_labels3 = None
        ...
        self.colors = { 'histogram'      : 4342452,
                         ...
                         'needle_fill'    : None,
                         'needle_outline2': 11829826,
                         'needle_fill2'   : None,
                         'needle_outline3': 11829826,
                         'needle_fill3'   : None,
                         'marker_outline2': 11829826,
                         'marker_fill2'   : None,
                         'text'           : 11829826,
                         'text2'          : 11829826,
                         'text3'          : 11829826}
    ...
    def add_needle2(self, value, needle_outline_color=None, needle_fill_color=None):
        """Draws a needle pointing towards value.
        needle_outline_color overrides the default"""
        self.gauge_value2 = value

        if needle_outline_color is not None:
            self.colors['needle_outline2'] = needle_outline_color

        if needle_fill_color is not None:
            self.colors['needle_fill2'] = needle_fill_color

    def add_needle3(self, value, needle_outline_color=None, needle_fill_color=None):
        """Draws a needle pointing towards value.
        needle_outline_color overrides the default"""
        self.gauge_value3 = value

        if needle_outline_color is not None:
            self.colors['needle_outline3'] = needle_outline_color

        if needle_fill_color is not None:
            self.colors['needle_fill3'] = needle_fill_color

    def check_needle_color(self):
        if self.gauge_value2 is not None and self.gauge_value is not None:
            if abs(self.gauge_value - self.gauge_value2) < 1:
                self.colors['needle_fill'] = None
                self.colors['needle_fill2'] = None

        if self.gauge_value3 is not None and self.gauge_value is not None:
            if abs(self.gauge_value - self.gauge_value3) < 1:
                self.colors['needle_fill'] = None
                self.colors['needle_fill3'] = None

        if self.gauge_value2 is not None and self.gauge_value3 is not None:
            if abs(self.gauge_value2 - self.gauge_value3) < 1:
                self.colors['needle_fill2'] = None
                self.colors['needle_fill3'] = None

    def add_marker(self, value, needle_outline_color=None, needle_fill_color=None):
        """Draws a marker pointing towards value.
        needle_outline_color overrides the default"""
        self.marker_value = value

        if needle_outline_color is not None:
            self.colors['marker_outline'] = needle_outline_color

        if needle_fill_color is not None:
            self.colors['marker_fill'] = needle_fill_color
                
    def add_marker2(self, value, needle_outline_color=None, needle_fill_color=None):
        """Draws a marker pointing towards value.
        needle_outline_color overrides the default"""
        self.marker_value2 = value

        if needle_outline_color is not None:
            self.colors['marker_outline2'] = needle_outline_color

        if needle_fill_color is not None:
            self.colors['marker_fill2'] = needle_fill_color

    def add_marker3(self, value, needle_outline_color=None, needle_fill_color=None):
        """Draws a marker pointing towards value.
        needle_outline_color overrides the default"""
        self.marker_value3 = value

        if needle_outline_color is not None:
            self.colors['marker_outline3'] = needle_outline_color

        if needle_fill_color is not None:
            self.colors['marker_fill3'] = needle_fill_color
    ...
    def add_text2(self, text_list = None, text_font_size=20,
                text_font=None, text_color=None):
        """Adds multiple lines of text as a caption.
        Usually used to display the value of the gauge.
        If label_list is not set, will create a single line label based on the value the needle is pointing to
        ...
        if text_list is None:
            if self.gauge_value2 is None:
                # Not enough information to do anything useful
                return
            else:
                text_list = str(self.gauge_value2)

        self.text_labels2 = []

        if type(text_list) is tuple:
            for l in text_list:
                self.text_labels2.append(l)
        else:
                self.text_labels2.append(text_list)
        ...
        if text_color is not None:
            self.colors['text2'] = text_color
            
    def add_text3(self, text_list = None, text_font_size=20,
                text_font=None, text_color=None):
        """Adds multiple lines of text as a caption.
        Usually used to display the value of the gauge.
        If label_list is not set, will create a single line label based on the value the needle is pointing to
        ...
        if text_list is None:
            if self.gauge_value3 is None:
                # Not enough information to do anything useful
                return
            else:
                text_list = str(self.gauge_value3)

        self.text_labels3 = []

        if type(text_list) is tuple:
            for l in text_list:
                self.text_labels3.append(l)
        else:
                self.text_labels3.append(text_list)
    ...
    def draw_labels2(self):
        """Draws the reading/text label 2"""
        if self.text_labels2 is not None:
            vstep = self.text_font_size * 1.3
            vpos = self.gauge_origin[1] + self.radius * 0.42 - (vstep * (len(self.text_labels2)-2)) / 2

            for l in self.text_labels2:
                text = unicode(l.encode("utf-8"), 'utf8')
                textsize = self.text_font.getsize(text)

                self.text((self.gauge_origin[0] - (textsize[0] / 2), vpos), text,
                          font=self.text_font, fill=self.colors['text2'])
                vpos += vstep
                         
    def draw_labels3(self):
        """Draws the reading/text label 3"""
        if self.text_labels3 is not None:
            vstep = self.text_font_size * 1.3
            vpos = self.gauge_origin[1] + self.radius * 0.42 - (vstep * (len(self.text_labels3)-4)) / 2

            for l in self.text_labels3:
                text = unicode(l.encode("utf-8"), 'utf8')
                textsize = self.text_font.getsize(text)

                self.text((self.gauge_origin[0] - (textsize[0] / 2), vpos), text,
                          font=self.text_font, fill=self.colors['text3'])
                vpos += vstep
    ...
    def draw_needle2(self):
        """Draws the needle 2"""
        if self.gauge_value2 is not None:

            if self.gauge_value2 < self.min_value:
                self.gauge_value2 = self.min_value

            if self.gauge_value2 > self.max_value:
                self.gauge_value2 = self.max_value

            angle = math.radians(self.min_angle + (self.gauge_value2 - self.min_value) *
                                 (self.max_angle - self.min_angle) / (self.max_value - self.min_value)
                                 + self.offset_angle)
            ...
            self._thick_polygon((left_point, end_point, right_point, mid_point), outline=self.colors['needle_outline2'],
                         fill=self.colors['needle_fill2'], thickness=self.dial_thickness)
                         
    def draw_needle3(self):
        """Draws the needle 3"""
        if self.gauge_value3 is not None:

            if self.gauge_value3 < self.min_value:
                self.gauge_value3 = self.min_value

            if self.gauge_value3 > self.max_value:
                self.gauge_value3 = self.max_value

            angle = math.radians(self.min_angle + (self.gauge_value3 - self.min_value) *
                                 (self.max_angle - self.min_angle) / (self.max_value - self.min_value)
                                 + self.offset_angle)
            ...
            self._thick_polygon((left_point, end_point, right_point, mid_point), outline=self.colors['needle_outline3'],
                         fill=self.colors['needle_fill3'], thickness=self.dial_thickness)
                         
    def draw_marker(self):
        """Draws the marker"""
        if self.marker_value is not None:
            
            if self.marker_value < self.min_value:
                self.marker_value = self.min_value

            if self.marker_value > self.max_value:
                self.marker_value = self.max_value            
            
            angle = math.radians(self.min_angle + (self.marker_value - self.min_value) *
                                 (self.max_angle - self.min_angle) / (self.max_value - self.min_value)
                                 + self.offset_angle)

            end_point = (self.gauge_origin[0] - self.radius * math.sin(angle) * 1.1, self.gauge_origin[1]
                        + self.radius * math.cos(angle) * 1.1)
            left_point = (self.gauge_origin[0] - self.radius * math.sin(angle - math.pi * 0.98) * -0.95,
                         self.gauge_origin[1] + self.radius * math.cos(angle - math.pi * 0.98) * -0.95)
            right_point = (self.gauge_origin[0] - self.radius * math.sin(angle + math.pi * 0.98) * -0.95,
                          self.gauge_origin[1] + self.radius * math.cos(angle + math.pi * 0.98) * -0.95)
            mid_point = (self.gauge_origin[0] - self.radius * math.sin(angle + math.pi) * -0.9,
                        self.gauge_origin[1] + self.radius * math.cos(angle + math.pi) * -0.9)                  
                             
            self._thick_polygon((left_point, end_point, right_point, mid_point), outline=self.colors['marker_outline'],
                         fill=self.colors['marker_fill'], thickness=self.dial_thickness)
                         
    def draw_marker2(self):
        """Draws the marker 2"""
        if self.marker_value2 is not None:
            
            if self.marker_value2 < self.min_value:
                self.marker_value2 = self.min_value

            if self.marker_value2 > self.max_value:
                self.marker_value2 = self.max_value            
            
            angle = math.radians(self.min_angle + (self.marker_value2 - self.min_value) *
                                 (self.max_angle - self.min_angle) / (self.max_value - self.min_value)
                                 + self.offset_angle)
            ...
            self._thick_polygon((left_point, end_point, right_point, mid_point), outline=self.colors['marker_outline2'],
                         fill=self.colors['marker_fill2'], thickness=self.dial_thickness)
                         
    def draw_marker3(self):
        """Draws the marker 3"""
        if self.marker_value3 is not None:
            
            if self.marker_value3 < self.min_value:
                self.marker_value3 = self.min_value

            if self.marker_value3 > self.max_value:
                self.marker_value3 = self.max_value            
            
            angle = math.radians(self.min_angle + (self.marker_value3 - self.min_value) *
                                 (self.max_angle - self.min_angle) / (self.max_value - self.min_value)
                                 + self.offset_angle)
            ...
            self._thick_polygon((left_point, end_point, right_point, mid_point), outline=self.colors['marker_outline3'],
                         fill=self.colors['marker_fill3'], thickness=self.dial_thickness)
